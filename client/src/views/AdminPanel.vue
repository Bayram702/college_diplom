      ...
      <div v-if="activeTab === 'users'" class="tab-content active">
        <div class="specialities-header">
          <div class="search-box">
            ...
            <input v-model="userSearch" type="text" placeholder="Поиск представителей..." @input="debouncedSearch">
          </div>
          ...
          <form class="sector-form" @submit.prevent="saveSector">
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input v-model="sectorForm.code" type="text" class="form-control" placeholder="09" inputmode="numeric" required>
                <small class="form-hint">Например, для 09.02.07 укажите 09.</small>
              </div>
            </div>
            <div class="form-group">
...
const userFilters = ref({ status: 'all' })
...
const fetchSectors = async () => {
  ...
  try {
    const response = await axios.get(`${API_URL}/sectors?include_inactive=1`)
...
const saveSector = async () => {
  const code = String(sectorForm.value.code || '').trim().match(/^\d+/)?.[0] || ''
