// Справочник специальностей СПО по приказу Минобрнауки России N 1199.
// Источник: ГАРАНТ / Национальный информационный центр, раздел "Перечень специальностей СПО".
export const spoSpecialtyGroups = [
  {
    code: '05.00.00',
    name: 'НАУКИ О ЗЕМЛЕ',
    specialties: [
  ...
  {
    code: '57.00.00',
    name: 'ОБЕСПЕЧЕНИЕ ГОСУДАРСТВЕННОЙ БЕЗОПАСНОСТИ',
    specialties: [
      ['57.02.01', 'Пограничная деятельность (по видам деятельности)']
    ]
  }
]

export const findSpoSpecialty = (code) => {
  for (const group of spoSpecialtyGroups) {
    const item = group.specialties.find(([specialtyCode]) => specialtyCode === code)
    if (item) {
      return { sectorCode: group.code, sectorName: group.name, code: item[0], name: item[1] }
    }
  }
  return null
}
